...
import com.dev.restart.auth.service.CustomUserDetailsService;
import com.dev.restart.global.token.JwtAccessDeniedHandler;
import com.dev.restart.global.token.JwtAuthenticationEntryPoint;
import com.dev.restart.global.token.JwtAuthenticationFilter;
...
//                        .requestMatchers("/api/v1/auth/login/**").permitAll()
                        .requestMatchers("/api/v1/**").permitAll() // 시간 부족으로 스프링 시큐리티를 통한 보안은 적용하지 않음
                        .requestMatchers( "/swagger-ui/**", "/v3/api-docs/**").permitAll() // swagger 접근 권환
